// Groq AI via backend proxy — API key is server-side only
...
export async function groqChat(messages, opts = {}) {
  ...
  if (!res.ok) {
    ...
    throw new Error(err?.error?.message || err?.error || `Groq error: ${res.status}`)
  }
...
- No extra text, just valid JSON.`
  const raw = await groqChat([{ role: 'user', content: prompt }], { temperature: 0.8, max_tokens: 900 })
...
{"relevance":<0-100>,"technical":<0-100>,"grammar":<0-100>,"confidence":<0-100>,"overall":<0-100>,"feedback":"<2-sentence assessment>","tips":["<tip1>","<tip2>","<tip3>"]}`
  const raw = await groqChat([{ role: 'user', content: prompt }], { temperature: 0.3, max_tokens: 400 })
  const match = raw.match(/\{[\s\S]*\}/)
...
{"ats_score":<0-100>,"extracted_skills":["..."],"missing_skills":["..."],"strengths":["..."],"weaknesses":["..."],"suggestions":["..."],"detected_role":"...","experience_level":"Fresher|Junior|Mid-Level|Senior"}`
  const raw = await groqChat([{ role: 'user', content: prompt }], { temperature: 0.2, max_tokens: 800 })
  const match = raw.match(/\{[\s\S]*\}/)
...
{"summary":"<2 sentences>","weak_areas":["...","..."],"action_plan":["...","...","...","..."],"resources":[{"title":"...","type":"Course|Book|Practice","url":"#"}]}`
  const raw = await groqChat([{ role: 'user', content: prompt }], { temperature: 0.4, max_tokens: 600 })
  const match = raw.match(/\{[\s\S]*\}/)
